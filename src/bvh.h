...
class bvh_node : public hittable {
	...
	public:
		...
		bvh_node(const std::vector<std::shared_ptr<hittable>>& src_objects, size_t start, size_t end) {
			...
			if (object_span == 1) {
				// List contains one object,
				// Set the left and right child as this object, don't recurse.
				left = right = objects[start];
			} else if (object_span == 2) {
				// List contains two objects,
				// figure out which should be in the left/right.
				...
				// Sort just the span we need _in place_
